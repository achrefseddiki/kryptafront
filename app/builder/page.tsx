        ...
        <div className="flex flex-col lg:flex-row gap-6 lg:gap-8 items-start">
          ...
          <div className="flex-1 flex flex-col gap-3 lg:gap-4 w-full">
            ...
            {components.map(({ slot, label, selected }, idx) => (
              <div
                key={slot}
                className={`bg-[#1a1a1a] border rounded-2xl p-4 lg:p-6 flex items-center justify-between gap-3 lg:gap-4 transition-colors ${
                  selected
              ...
              >
                <div className="flex items-center gap-3 lg:gap-5 min-w-0">
                  ...
                  <div className="min-w-0">
                    ...
                    {selected ? (
                      <p className="text-white text-sm lg:text-base font-medium mt-0.5 truncate">{selected}</p>
                    ) : (
              ...
              <div className="flex flex-col gap-3">
                {components.filter((c) => c.selected).map(({ slot, label, selected }) => (
                  <div key={slot} className="flex justify-between text-sm gap-4">
                    ...
                    <span className="text-white text-right text-xs truncate max-w-[160px]">{selected}</span>
                  </div>
                ))}
              </div>
